  ...
  return (
    <div className="relative w-full max-w-2xl mx-auto px-4 md:px-0">
      <div className="relative">
        ...
        <Input
          type="text"
          placeholder={isRecording ? "Listening..." : "Song, movie, artist, or lyrics..."}
          value={value}
          onChange={(e) => onChange(e.target.value)}
          disabled={isRecording || isProcessing}
          className={`search-input w-full h-12 md:h-14 pl-12 pr-24 text-base md:text-lg rounded-2xl bg-secondary/50 border-border/50 placeholder:text-muted-foreground/60 focus-visible:ring-primary/30 transition-all duration-300 ${
            isRecording ? "border-primary/50 ring-2 ring-primary/30 bg-primary/5" : ""
        ...
        <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
          ...
          {/* Recording time */}
          {isRecording && (
            <span className="text-xs font-mono text-primary mr-1">{formattedTime}</span>
          )}
